//LCP(Longest common prefix) 배열 : 접미사 배열에서 현재 접미사와 이전 접미사의 최장 공통 접두사를 저장하는 배열
//(경우에 따라 현재 접미사와 다음 접미사의 최장 공통 접두사를 구할수도 있음)
...
class Lecture{
    ...
    public static void Main(string[] args){
        ...
        Console.WriteLine($"서로 다른 부분 문자열의 수 = {DifferentSubstrings(str)}");
    }
    ...
    //문자열의 서로 다른 부분 문자열의 수를 계산하기 위해서는 모든 부분 문자열의 수에서
    //LCP배열 값의 합을 빼면 됨
    public static int DifferentSubstrings(string str){
        int sum = 0;
        for(int i = 0; i < LCP.Length; i++){
            sum += LCP[i];
        }
        
        return ((LCP.Length*(LCP.Length+1)) / 2)  - sum;
    }
